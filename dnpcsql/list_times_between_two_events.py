...
if __name__ == "__main__":
    ...
    for (root_span_uuid, events_iterator) in groups:
        groups_count += 1
        # print(f"Processing root span {root_span_uuid}")
        events = list(events_iterator)
        # print(f"This span has {len(events)} events")
        #parsl.tracing.PACKAPPLY/SERIALIZE_PACK_APPLY_FUNC
        #parsl.tracing.PACKAPPLY/SERIALIZE_PACK_APPLY_FUNC
        #parsl.tracing.PACKAPPLY/SERIALIZE_PACK_APPLY_ARGS
        start_events = [e for e in events if e[3] == "SERIALIZE_PACK_APPLY_FUNC"]
        end_events = [e for e in events if e[3] == "SERIALIZE_PACK_APPLY_ARGS"]

        if start_events == [] or end_events == []:
            skip_count += 1
            continue

        if len(start_events) > 1:
            print("WARNING: more than one start event in this root sequence")
        if len(end_events) > 1:
            print("WARNING: more than one end event in this root sequence")

        start_event = start_events[0]
        end_event = end_events[0]
        start_t = float(start_event[1])
        end_t = float(end_event[1])

        delta_t = end_t - start_t
        print(f"{start_t} {delta_t}")

    print(f"Skipped {skip_count} out of {groups_count} root sequences")
    import sys
    sys.exit()
